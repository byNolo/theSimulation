...
const StatBar: React.FC<{ label: string; value: number; color: string; max?: number }> = ({ label, value, color, max = 100 }) => {
  ...
  const getStatInfo = () => {
    ...
    if (labelLower === 'production') {
      return {
        title: '🏗️ Base Production',
        description: 'The daily industrial output of your settlement. Used to construct projects and rebuild civilization.',
        ranges: [
          { range: '0-9', label: 'Stagnant', effect: 'Little to no progress on projects.', color: 'text-red-400' },
          { range: '10-24', label: 'Active', effect: 'Steady construction pace.', color: 'text-yellow-400' },
      ...
      >
        ...
        <div className="h-3 bg-black/30 rounded-full overflow-hidden relative">
          <div
            ...
            style={{ width: `${Math.min(100, (value / max) * 100)}%`, background: `linear-gradient(90deg, ${color}, ${color}dd)` }}
          >
        ...
        <div className="mt-2 text-xs text-gray-500 font-medium">
          ...
          ) : label.toLowerCase() === 'production' ? (
            <>
              {value < 10 && '⚠️ Stagnant'}
              {value >= 10 && value < 25 && '⚡ Active'}
              {value >= 25 && '🚀 Booming'}
            </>
          ) : (
            <>
